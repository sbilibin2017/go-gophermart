...
func getLoginFromContext(
	w http.ResponseWriter,
	r *http.Request,
	loginProvider func(ctx context.Context) (string, error),
) (string, error) {
	login, err := loginProvider(r.Context())
	if err != nil {
		sendTextPlainResponse(w, errUnauthorized.Error(), http.StatusUnauthorized)
		return "", err
...
func decodeRequestBody(w http.ResponseWriter, r *http.Request, req interface{}) error {
	...
	if err != nil {
		sendTextPlainResponse(w, errInvalidRequestBody.Error(), http.StatusBadRequest)
		return err
	}
	return nil
}
...
const (
	errFailedToEncodeResponseBody = "Failed to encode response"
)
