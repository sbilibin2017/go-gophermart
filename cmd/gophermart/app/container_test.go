...
func TestContainer_ConfigMethods(t *testing.T) {
	...
	defer ctrl.Finish()
	mockConfig := NewMockContainerConfig(ctrl)
	mockConfig.EXPECT().GetAccrualSystemAddress().Return("http://accrual-system.local").Times(1)
	...
	assert.Equal(t, "http://accrual-system.local", mockConfig.GetAccrualSystemAddress())
